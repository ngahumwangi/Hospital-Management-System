...
namespace pharmSoft
{
    public partial class Sales : Form
    {
        ...
        public string GetUniqueKey()
        {
            ...
            con.Open();
            string Query = "select max(Sales_Id) from sales";//selection string
        ...
        void pdoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            try
            {
                ...
                graphics.DrawString(" KANGUNDO MEDICAL CENTRE", new Font("Constantia", 12), new SolidBrush(Color.Black), startX, startY + Offset);
               Offset = Offset + 20;
                graphics.DrawString(kk + "KANGUNDO", new Font("Courier New", 12), new SolidBrush(Color.Black), startX, startY + Offset);
               Offset = Offset + 20;
               graphics.DrawString(kk + "User"+userid.Text, new Font("Courier New", 12), new SolidBrush(Color.Black), startX, startY + Offset);
               Offset = Offset + 20;
               ...
               graphics.DrawString(blankspace, new Font("Constantia", 8), new SolidBrush(Color.Black), startX, startY + Offset);
                ...
                Offset = Offset + 20;
                graphics.DrawString(kk + "WE TREAT,GOD HEALS" + space, new Font("Constantia", 8), new SolidBrush(Color.Black), startX, startY + Offset);
                Offset = Offset + 20;
                graphics.DrawString(blankspace, new Font("Constantia", 8), new SolidBrush(Color.Black), startX, startY + Offset);
                Offset = Offset + 20;
        ...
        public int CheckQuantity()
        {
            ...
            {
                MessageBox.Show("Could not get Quantity of the selected product"+ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return quantity;
        }
            ...
            try
            {
                    ...
                    for (int j = 0; j <= ListView1.Items.Count - 1; j++)
                {
                    ...
                    try
                    {
                        ...
                        string Query = "insert into sells(modeofpayment,total,discount,cash,datetime) VALUES ('" + cbopayment.SelectedItem + "','" + Convert.ToDouble(txtTotal.Text) + "','" + Convert.ToDouble(txtDiscount.Text) + "','" + Convert.ToDouble(txtcash.Text) + "','" + DateTime.Now.ToString("yyy-MM-dd HH:mm:ss") + "')";
                        MySqlCommand mycommand1 = new MySqlCommand(Query, con);
                    ...
                    {
                        MessageBox.Show("Could Not save into sales: " + ex.Message);
                    }
                    ...
                    for (int i = 0; i <= ListView1.Items.Count - 1; i++)
                    {
                        ...
                        {
                            MessageBox.Show("For loop Error" + ex);
                        }
                    }
