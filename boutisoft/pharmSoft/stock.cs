...
namespace pharmSoft
{
    public partial class Stock : Form
    {
       ...
       Eangenerate();//fuction to return the barcode value
        ...
        else
        {
            try
            {
                ...
                MySqlConnection myconn1 = new MySqlConnection(cs.myconnection1);
                myconn1.Open();
                string Query1 = "insert into stockdetails(productname,batchno,buyingprices,quantity,desctription,manufacturer,barcode,sellingprices,datetime,expirydate) values('" + txtproductname.Text + "','" + txtbatchno.Text + "','" + Convert.ToDecimal(txtbuyingprice.Text) + "','" + Convert.ToInt64(txtquantity.Text) + "','" + txtpackage.Text + "','" + txtmanufacture.Text + "','" + Convert.ToInt64(txtbarcode.Text) + "','"+ Convert.ToDecimal(txtsellingprice.Text)+"','" + DateTime.Now.ToString("yyyy-MM-dd") + "','" + date.ToString("yyyy-MM-dd") + "')";
       ...
       private void txtmanufacture_TextChanged(object sender, EventArgs e)
       {
           try
           {
               string query = "select manufacturer from stockdetails where barcode like '%" + txtmanufacture.Text + "%'";
               MySqlConnection myconn = new MySqlConnection(cs.myconnection1);
               myconn.Open();
